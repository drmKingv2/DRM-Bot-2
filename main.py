...
# Config 
class Config(object):
    BOT_TOKEN = os.environ.get("BOT_TOKEN", "6807398227:AAE6h-aJAqsVVpBLKY1dkIOFHu03O8bGoEg")
    API_ID = int(os.environ.get("API_ID",  "27760436"))
    API_HASH = os.environ.get("API_HASH", "8a3ceb4c4937b25d0af363b498f4d3a8")
    DOWNLOAD_LOCATION = "./DOWNLOADS"
    SESSIONS = "./SESSIONS"

    AUTH_USERS = os.environ.get('AUTH_USERS', '6526760264').split(',')
    for i in range(len(AUTH_USERS)):
        AUTH_USERS[i] = int(AUTH_USERS[i])

    GROUPS = os.environ.get('GROUPS', '-1002168220107').split(',')
    for i in range(len(GROUPS)):
        GROUPS[i] = int(GROUPS[i])

    LOG_CH = os.environ.get("LOG_CH", "-1002168220107")
